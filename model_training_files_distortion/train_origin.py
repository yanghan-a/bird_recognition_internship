...
def plot_loss(train_loss, val_loss, epochs, save_path,val_accurate_values):
    ...
    plt.legend()
    plt.savefig(os.path.join(save_path, 'loss_curve_origin.png'))
    plt.show()
    plt.figure()
    ...
    plt.legend()
    plt.savefig(os.path.join(save_path, 'accuracy_origin.png'))
    plt.show()
...
def main():
    ...
    image_path = os.path.join(data_root, "../data_set", "new_data_distortion")  # flower data set path
    ...
    best_acc = 0.0
    save_path = './model_origin/MobileNetV3_Small_distortion.pth'
